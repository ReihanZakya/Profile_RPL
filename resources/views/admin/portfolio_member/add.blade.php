    ...
    <div class="card card-primary">
        ...
        <form action="{{ url('portfolio_member/store') }}" method="POST">
            ...
            <div class="card-body">
                    ...
                    <select class="form-control custom-select @error('portfolio_id') is-invalid @enderror" name="portfolio_id" id="exampleFormControlSelect1">
                      <option selected="" disabled="">---Pilih Portofolio---</option>
                        @foreach ($portfolio as $pt)
                            <option value="{{ $pt->id }}" {{ old('portfolio_id') == $pt->id ? 'selected' : '' }}>{{ $pt->name }}</option>
                        @endforeach
                    </select>
                    @error('portfolio_id')
                ...
                <!-- Mata Pelajaran -->
                <div class="form-group">
                <label for="exampleFormControlSelect2">Pilih Anggota</label>
                <select multiple class="form-control custom-select @error('member_id') is-invalid @enderror" name="member_id[]" id="exampleFormControlSelect2">
                    @foreach ($member as $mb)
                        <option value="{{ $mb->id }}" {{ (collect(old('member_id'))->contains($mb->id)) ? 'selected' : '' }}>{{ $mb->member_name }}</option>
                    @endforeach
                  </select>
                  @error('member_id')
                    <div class="invalid-feedback">{{ $message }}</div>
